...
const findNums = (line, index) => {
    ...
    while(line.substring(start, start + 1).match(/\d/)) {
        if (start == 0) break
        start--
    }
    let end = Math.min(index + 1, line.length - 1)
    while(line.substring(end, end + 1).match(/\d/)) {
        if (end == line.length) break
        end++
    }
    const strip = line.substring(start, end).replaceAll(/\D/g, ' ').trim()
    const result = strip.length ? strip.split(' ') : []
    return result
}
...
const part1 = input => input.split('\n').reduce((result, line, row, lines) => {
    for (const match of line.matchAll(/(\d+)/g)) {
        ...
        if (patch.replaceAll(/[0-9\.]/g, '').length > 0) result += 1 * match[0]
    }
    return result
}, 0)
...
const part2 = input => input.split('\n').reduce((result, line, row, lines) => {
    for (const match of line.matchAll(/([\*])/g)) {
        let connects = []
        if (row) {
            for (const num of findNums(lines[row - 1], match.index)) connects.push(num)
        }
